...
# standing NOTHING else here goes red. Writers that take their agent id from
# the ConfigMap (finops runner, console) get sealed under
# `agent://set-me.invalid/...` with `foreign_trust_domain 0`, a signed history
# under a domain nobody owns; writers that carry their own id (the gateway's
# callers, the drills) are refused as foreign, the quiet night GOTCHAS 90
# describes. Neither is red anywhere else, which is why this line exists: the
# placeholder has to be red somewhere an operator looks, and this is where
# they look. `.invalid` is the RFC 2606 reserved TLD, so no real domain ends
# in it.
if domain="$(kc get cm stack-wiring -o jsonpath='{.data.TRAILRYX_TRUST_DOMAIN}' 2>&1)"; then
  case "$domain" in
    ""|*.invalid)
  ...
  bad "could not read ConfigMap stack-wiring, so the trust domain is unknown: $domain"
fi
