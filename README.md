...
| CronJob | three of `routines.sh`'s five governance routines: crypto trend, quality drift, identity sweep (`qryx-trend`, `verdryx-drift`, `idryx-detect`), plus the record seal (`trailryx-seal`, at 5:27, last because it seals what the others did). FinOps export cannot run as a CronJob here, its data lives in a per-pod `emptyDir` (GOTCHAS 81); `drills` (`mockryx-drill`) is separate: opt-in, suspended below | |
